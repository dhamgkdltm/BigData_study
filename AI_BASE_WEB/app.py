...
def uploader_file():
    f = request.files['file'] # index.html에서 file이라는 이름의 요소가 가지고 있는 8.png를 객체로 호출함

    print(f.filename) # f 객체에서 filename을 가져옴, "8.png"라고 되어있을 것이다.
    f.save("./images/"+f.filename) # f를 현재 작업디렉토리 images 폴더에 8.png라고 저장
